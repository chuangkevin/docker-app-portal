...
export async function runMigrations(db: DrizzleDb): Promise<void> {
  ...
  // Add display_name column to services if it doesn't exist
  try {
    db.run(sql`ALTER TABLE services ADD COLUMN display_name TEXT`);
  } catch {
    // Column already exists, ignore
  }

  // Add is_external column to services if it doesn't exist
  try {
    db.run(sql`ALTER TABLE services ADD COLUMN is_external INTEGER NOT NULL DEFAULT 0`);
  } catch {
    // Column already exists, ignore
  }
